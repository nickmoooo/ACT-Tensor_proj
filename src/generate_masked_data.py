...
def load_skip_mask(skipped_path: str, N: int) -> np.ndarray:
    """Return (N,) bool mask: True = skipped firm."""
    idx = np.load(skipped_path)
...
if __name__ == '__main__':
    ...
    data = np.load('/Users/nickmoooo/Desktop/Research/SPUR/missing_data_pub-main/data/raw_chars.npz')
    # data = np.load('/Users/nickmoooo/Desktop/Research/SPUR/missing_data_pub-main/data/raw_rank_trunk_chars.npz')
    percentile_rank_chars = data['rank_chars']
    regular_chars = data['raw_chars']
    chars = data['chars']
    # chars = [
    #     'A2ME','AT','ATO','B2M','BETA_d','BETA_m','C2A','CF2B','CF2P','CTO',
    #     'D2A','D2P','DPI2A','E2P','HIGH52','INV','IdioVol','LEV','ME','NI',
    ...
    # skip_mask = load_skip_mask("skipped_companies.npy", N)

    # percentile_rank_chars[:, skip_mask, :] = np.nan
    # return_panel[:, skip_mask] = np.nan
    
    # sic_fic = pd.read_csv("../data/sic_fic.csv")
    sic_fic = pd.read_csv('/Users/nickmoooo/Desktop/Research/SPUR/missing_data_pub-main/data/sic_fic.csv')
