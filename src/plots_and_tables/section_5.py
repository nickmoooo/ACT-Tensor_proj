...
class AggregateImputationErrorsFullDataset(SectionFiveTableBase):
    ...
    def setup(self, percentile_rank_chars, chars, monthly_updates):
        ...
        plot_names = [
            ...
            # "cp_all",
            # "cp_all_center_ma",
            # "tucker_pooling_smooth_all_bw",
            # "tucker_pooling_smooth_all_bw_kalman_filter",
            # "tucker_pooling_smooth_all_fwbw",
            # "tucker_pooling",
            # "tucker",
            # "cp_pooling",
            # "cp",
        ...
        def run_tag(tag):
            ...
            def func_call(group, eval_data, groups):
                ...
                return imputation_utils.get_imputation_metrics(data, 
                                      ...
                                      char_groupings=groups, norm_func=None, tag=tag, clip=True)

            metrics = [func_call(group, eval_chars, char_groupings) for group in subsets] #LOOK AT HERE !!!!!!!!
        ...
        def run_tag(tag):
            ...
            def plot_metrics_by_mean_vol(mean_vols, input_metrics, names, chars, save_name=None):
                ...
                for j, (c, line_name, metrics_series) in enumerate(zip(mycolors, names,
                    ...
                    plt.plot(np.arange(43), metrics_series, label=line_name, c=c,
                            linestyle=imputation_utils.line_styles[j])
                plt.plot(np.arange(43), np.array(mean_vols)[np.argsort(mean_vols)], label="mean volatility of char", c='black')
                plt.xticks(np.arange(43), chars[np.argsort(mean_vols)], rotation='vertical')
                plt.ylabel("R2")
...
class ImputationErrorPlots(SectionFiveTableBase):
    ...
    def setup(self, percentile_rank_chars, chars, monthly_updates, dates, plot_over_time=False, table_1=True):
        ...
        # date_vals = np.array(dates) // 10000 + ((np.array(dates) // 100) % 100) / 12
        date_vals = di.year + (di.month - 1) / 12.0
        update_chars = np.copy(percentile_rank_chars)
        for i, c in enumerate(chars):
            if char_map[c] !='M':
                update_chars[~(monthly_updates == 1),i] = np.nan
        ...
        subsets = [
            # ['partial_tucker'],
            # ['tensor_train_pooling'],
            # ['tensor_train'],
            # ['tucker_pooling_smooth_linear_interp'],
            # ['tucker_pooling_smooth_arima'],
            # ['tucker_pooling_smooth_ema'],
            # ['tucker_pooling_smooth_center_ma3'],
            # ['tucker_pooling_smooth_center_ma5'],
            # ['tucker_pooling_smooth_hp_filter'],
            # ['tucker_pooling_smooth_kalman_filter'],
            # ['tucker_pooling_smooth_all_bw'],
            # ['tucker_pooling_smooth_all_bw_kalman_filter'],
            # ['tucker_pooling_smooth_all_fwbw'],
            # ['tucker_pooling'],
            ...
            ["prev_val", 'xs_median'], 
            ['xs_median'],             
            ['tensor_train_pooling_smooth_all'],
            ['tensor_train_pooling_smooth_all_ema'],
            ['tensor_train_pooling_smooth_all_center_ma'],
            ['tensor_train_pooling_smooth_all_kalman_filter'],
            ['tucker_pooling_smooth_all'],
            ['tucker_pooling_smooth_all_ema'],
            ['tucker_pooling_smooth_all_center_ma'],
            ['tucker_pooling_smooth_all_kalman_filter'],
            ['cp_pooling_smooth_all'],
            ['cp_pooling_smooth_all_ema'],
            ['cp_pooling_smooth_all_center_ma'],
            ['cp_pooling_smooth_all_kalman_filter'],
            ['em'],
        ]
        ...
        plot_names = [
            ...
            # "XS-median", 
            # "ind-median",
            # "Tensor Train Pooling",
            # "Tensor Train Pooling Smooth EMA",
            # "Tensor Train Pooling Smooth Center MA",
            # "Tensor Train Pooling Smooth Kalman Filter",
            # "Tucker Pooling Smooth",
            # "Tucker Pooling Smooth EMA",
            # "Tucker Pooling Smooth Center MA",
            # "Tucker Pooling Smooth Kalman Filter",
        ]
        
        
        # table_1_plotnames = ["global BF-XS", "local B-XS", "local XS",  "local B",  "XS-median", "ind-median"]
        # table_1_plotnames = ["global BF-XS", "local B-XS", "local XS",  "local B",  "XS-median", "ind-median", 
        #                         "tensor_train_pooling_smooth_all", "tensor_train_pooling_smooth_all_ema", 
        #                         "tensor_train_pooling_smooth_all_center_ma", "tensor_train_pooling_smooth_all_kalman_filter",
        #                         "tucker_pooling_smooth_all", "tucker_pooling_smooth_all_ema", 
        #                         "tucker_pooling_smooth_all_center_ma", "tucker_pooling_smooth_all_kalman_filter",
        #                         "cp_pooling_smooth_all", "cp_pooling_smooth_all_ema", 
        #                         "cp_pooling_smooth_all_center_ma", "cp_pooling_smooth_all_kalman_filter"]
        table_1_plotnames = ["global BF-XS", "local B-XS", "local B", 
                             "tensor_train_pooling_smooth_all",
                             "tucker_pooling_smooth_all",
                             "cp_pooling_smooth_all", 
                            ]
        ...
        [run_tag(tag) for tag in 
                               [
                                # '_in_sample', 
                                '_out_of_sample_MAR', '_out_of_sample_block', '_out_of_sample_logit']]
...
class ImputationErrorsByCharQuintile(SectionFiveTableBase):
    ...
    def setup(self, percentile_rank_chars, chars, monthly_updates):
        file_names = ['cp', 'cp_pooling', 'tucker', 'tucker_pooling', 'tensor_train',
                     'global_fwbw', 'global_fw', 'global_bw', 'global_xs',
                     'global_ts', 'local_bw', 'local_xs', 'prev_val', 'local_ts',
                     'xs_median', 'indus_median']
        plot_names = ['cp', 'cp_pooling', 'tucker', 'tucker_pooling', 'tensor_train',
                    "global BF-XS", "global F-XS", "global B-XS", "global XS",
                      "global B",
...
class InfoUsedForImputationBW(SectionFivePlotBase):
    ...
    def setup(self, percentile_rank_chars, return_panel, chars, monthly_updates, norm_regressors=False):
        ...
        plt.plot(np.arange(43), np.array(mean_acfs)[np.argsort(mean_acfs)], label="Autocorrelation")
        
        bw_betas[0,:,2] += bw_betas[0,:,1]
        norm = np.linalg.norm(bw_betas[0,:,:], axis=1, ord=1)
        plt.plot(np.arange(43), (np.abs(bw_betas[0,:,0]) /norm)[np.argsort(mean_acfs)], label="XS weight")
        
        plt.plot(np.arange(43),(np.linalg.norm(bw_betas[0,:,1:], axis=1, ord=1)/norm)[np.argsort(mean_acfs)], label="TS weight")
        
        plt.xticks(np.arange(43), chars[np.argsort(mean_acfs)], rotation='vertical')
        plt.minorticks_off()
        plt.ylabel("weight")
