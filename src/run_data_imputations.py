...
logging.getLogger('tensorflow').setLevel(logging.ERROR)  # Suppress retracing warnings
...
#     Global
    ...
    prev_val = imputation_model_simplified.simple_imputation(gamma_ts, masked_lagged_chars, 
                                             "fwbw", None, char_groupings,
                                                            eval_char_data=None)
    imputation_utils.save_imputation(prev_val, dates, permnos, chars, "prev_val_all" + tag)
    del prev_val
    ...
    imputation_utils.save_imputation(local_ts, dates, permnos, chars, "local_ts_all" + tag)
    del local_ts
    ...
    imputation_utils.save_imputation(xs_median, dates, permnos, chars, "xs_median_all" + tag)
    del xs_median
    ...
    imputation_utils.save_imputation(indus_median, dates, permnos, chars, "indus_median_all" + tag)
    del indus_median

    ##########################################################################################

    for smooth_method in ['cma']: 
        cp_pooling_smooth = imputation_model_simplified.impute_chars(
            masked_lagged_chars, None, None, 
            suff_stat_method='cp_pooling_smooth', 
            constant_beta=True, beta_weight=False,
            tag=tag, smooth_method=smooth_method,
            return_panel=return_panel,
        )
        if smooth_method == None:
            imputation_utils.save_imputation(cp_pooling_smooth, dates, permnos, chars, "cp_pooling_smooth" + tag)
        else:
            imputation_utils.save_imputation(cp_pooling_smooth, dates, permnos, chars, "cp_pooling_smooth_" + str(smooth_method) + tag)
        del cp_pooling_smooth
    

        cp_all = imputation_model_simplified.impute_chars(
            masked_lagged_chars, None, None, 
            suff_stat_method='cp', 
            constant_beta=True, beta_weight=False,
            tag = tag, smooth_method=smooth_method,
        ...
        if smooth_method == None:
            imputation_utils.save_imputation(cp_all, dates, permnos, chars, "cp" + tag)
        else:
            imputation_utils.save_imputation(cp_all, dates, permnos, chars, "cp_" + str(smooth_method) + tag)
...
if __name__ == '__main__':
    ...
    run_all_imputations(missing_data_type="MAR", percentile_rank_chars=percentile_rank_chars, 
                    ...
                    monthly_updates=monthly_updates, char_map=char_map)
